	...
	return elm$core$Platform$Sub$batch(
		_List_fromArray(
			[
				...
				A2(elm$time$Time$every, author$project$Config$config.snowflakeFrequency, author$project$Main$NewSnowflake)
			]));
};
...
var mgold$elm_animation$Animation$animate = F2(
	function (clock, _n0) {
		...
		var delay_ = _n0.a.delay_;
		var dos = _n0.a.dos;
		var ramp = _n0.a.ramp;
		var from_ = _n0.a.from_;
		var to_ = _n0.a.to_;
		var ease_ = _n0.a.ease_;
...
var author$project$Snowflake$animatePosition = F2(
	function (clock, position) {
		return {
			rotation: A2(mgold$elm_animation$Animation$animate, clock, position.rotation),
			x: A2(mgold$elm_animation$Animation$animate, clock, position.x),
			y: A2(mgold$elm_animation$Animation$animate, clock, position.y)
		};
	});
var author$project$Main$isFlakeCaught = F2(
	function (model, flakePosition) {
		var withinDistance = F2(
			function (a, b) {
				return _Utils_eq(
					A2(
						elm$core$Basics$compare,
						elm$core$Basics$abs(a - b),
	...
	function (msg, model) {
		switch (msg.$) {
			case 'Tick':
				...
				if (_n1.$ === 'Playing') {
					var startTime = _n1.a;
					var clock = model.clock + time;
					var won = _Utils_eq(
						A2(elm$core$Basics$compare, clock - startTime, author$project$Config$config.gameLengthSeconds * 1000),
						elm$core$Basics$GT);
					var _n2 = A2(
						elm$core$List$partition,
						author$project$Main$isFlakeCaught(model),
						model.snowflakePositions);
					var caught = _n2.a;
					var uncaught = _n2.b;
					var newScore = model.score + elm$core$List$length(caught);
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								...
								state: won ? author$project$Main$Won(newScore) : model.state
							}),
			...
			case 'NewSnowflake':
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							snowflakePositions: A2(
								elm$core$List$cons,
								position,
								A2(elm$core$List$take, author$project$Config$config.maxSnowflakes, model.snowflakePositions))
						}),
					A2(
						elm$random$Random$generate,
					...
					case 'Unstarted':
						return _Utils_Tuple2(
							_Utils_update(
								...
								{
									score: 0,
									state: author$project$Main$Playing(model.clock)
								}),
							elm$core$Platform$Cmd$none);
					case 'Won':
						return _Utils_Tuple2(
...
var the_sett$elm_color$Color$lightBlue = A4(the_sett$elm_color$Color$rgba, 114, 159, 207, 1);
...
var timjs$elm_collage$Collage$Text$style = F2(
	function (newstyle, _n0) {
		var str = _n0.b;
		return A2(timjs$elm_collage$Collage$Core$Chunk, newstyle, str);
	});
var author$project$Main$customStyle = timjs$elm_collage$Collage$Text$style(
	{
		color: the_sett$elm_color$Color$white,
		line: timjs$elm_collage$Collage$Text$None,
		shape: timjs$elm_collage$Collage$Text$Upright,
		size: 100,
		typeface: timjs$elm_collage$Collage$Text$Font(author$project$Config$config.font),
		weight: timjs$elm_collage$Collage$Text$Regular
	});
...
var author$project$Main$happyHolidays = timjs$elm_collage$Collage$Layout$vertical(
	_List_fromArray(
		[
			timjs$elm_collage$Collage$rendered(
			author$project$Main$customStyle(
				timjs$elm_collage$Collage$Text$fromString('Happy Holidays'))),
			A2(timjs$elm_collage$Collage$Layout$spacer, 0, 30),
			timjs$elm_collage$Collage$rendered(
			A2(
				timjs$elm_collage$Collage$Text$size,
				30,
				author$project$Main$customStyle(
					timjs$elm_collage$Collage$Text$fromString('Click to catch some flakes'))))
		]));
...
var author$project$Main$youWon = F2(
	function (points, totalFlakes) {
		return A2(
			...
			timjs$elm_collage$Collage$Layout$vertical(
				_List_fromArray(
					[
						timjs$elm_collage$Collage$rendered(
						author$project$Main$customStyle(
							timjs$elm_collage$Collage$Text$fromString('Nice!'))),
						timjs$elm_collage$Collage$rendered(
						A2(
							timjs$elm_collage$Collage$Text$size,
							30,
							author$project$Main$customStyle(
								timjs$elm_collage$Collage$Text$fromString(
									'You caught ' + (elm$core$String$fromInt(points) + ' flakes'))))),
						...
						A2(
							...
							30,
							author$project$Main$customStyle(
								timjs$elm_collage$Collage$Text$fromString('Click to play again'))))
					])));
	});
var timjs$elm_collage$Collage$Text$empty = timjs$elm_collage$Collage$Text$fromString('');
var author$project$Main$message = function (model) {
	var _n0 = model.state;
	switch (_n0.$) {
		case 'Unstarted':
			return author$project$Main$happyHolidays;
		case 'Playing':
			var startTime = _n0.a;
			return timjs$elm_collage$Collage$rendered(timjs$elm_collage$Collage$Text$empty);
		default:
			var points = _n0.a;
			return A2(author$project$Main$youWon, points, model.totalFlakes);
	}
};
...
var the_sett$elm_color$Color$lightGrey = A4(the_sett$elm_color$Color$rgba, 238, 238, 236, 1);
...
var author$project$Snowflake$snowflake = timjs$elm_collage$Collage$group(
	A2(
		elm$core$List$map,
		function (d) {
			return A2(
				...
				A2(
					...
					A2(
						...
						timjs$elm_collage$Collage$uniform(the_sett$elm_color$Color$white)),
					timjs$elm_collage$Collage$line(10)));
		},
		_List_fromArray(
			[45, 90, 135, 180])));
var timjs$elm_collage$Collage$shiftY = F2(
	function (dy, collage) {
		var _n0 = collage.shift;
		var x = _n0.a;
		var y = _n0.b;
		return _Utils_update(
			collage,
			{
				shift: _Utils_Tuple2(x, y + dy)
			});
	});
var author$project$Main$snowflakes = function (model) {
			...
			function (p) {
				return A2(
					...
					A2(
						...
						p.y,
						A2(timjs$elm_collage$Collage$shiftX, p.x, author$project$Snowflake$snowflake)));
			},
			A2(
				elm$core$List$map,
				author$project$Snowflake$animatePosition(model.clock),
				model.snowflakePositions)));
};
var elm$core$Basics$degrees = function (angleInDegrees) {
	return (angleInDegrees * elm$core$Basics$pi) / 180;
};
var the_sett$elm_color$Color$orange = A4(the_sett$elm_color$Color$rgba, 245, 121, 0, 1);
...
var author$project$Main$snowman = function (currentScore) {
	var whiteCircle = function (size) {
		return A2(
			timjs$elm_collage$Collage$filled,
			timjs$elm_collage$Collage$uniform(the_sett$elm_color$Color$white),
			timjs$elm_collage$Collage$circle(size));
	};
	var nose = A2(
		timjs$elm_collage$Collage$scaleX,
		3,
		A2(
			...
			A2(
				timjs$elm_collage$Collage$filled,
				timjs$elm_collage$Collage$uniform(the_sett$elm_color$Color$orange),
				timjs$elm_collage$Collage$triangle(20))));
	return timjs$elm_collage$Collage$group(
		A2(
			elm$core$List$indexedMap,
			F2(
				function (i, e) {
					return A2(timjs$elm_collage$Collage$shiftY, i * 90, e);
				}),
			_List_fromArray(
				[
					whiteCircle(90),
					...
					A2(timjs$elm_collage$Collage$shiftX, 10, nose),
